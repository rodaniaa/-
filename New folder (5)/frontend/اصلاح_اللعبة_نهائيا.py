...
if not os.path.exists(bank_dir):
    # Try alternate location if not in frontend
    bank_dir = os.path.join(os.path.dirname(current_dir), 'backend', 'بنك')

# We now target all_questions.js which is the file used in the new index.html
js_file = os.path.join(current_dir, 'all_questions.js')

print("--- محرك تحديث اللعبة الشامل (v3) ---")
if not os.path.exists(bank_dir):
    print(f"❌ خطأ: لم أستطع العثور على مجلد (بنك) في {bank_dir}")
    exit(1)

print(f"🎯 تم العثور على بنك الأسئلة في: {bank_dir}")
print(f"💾 سيتم الحفظ في: {js_file}")
...
def clean_ar(t): 
    # Helper to normalize Arabic for matching
...
def run():
    ...
    for f_name in files:
        ...
        for i, line in enumerate(lines):
            ...
            if 'سؤال' in line:
                ...
                # Look for answer in next 5 lines
                a_text = ""
  ...
  if (options.length === 0) {
    return { category: "bible", difficulty: "متنوع", question: "لا توجد أسئلة كافية لهذا الحرف!", answer: "-" };
  }
  ...
  if (sel.question && sel.question !== "لا توجد أسئلة كافية لهذا الحرف!") {
...
''')
        print(f"✅ تم التحديث بنجاح! إجمالي الأسئلة المستوردة: {total}")
        for l in letters_board:
            if stats[l] > 0:
                print(f"  - الحرف {l}: {stats[l]} سؤال")
