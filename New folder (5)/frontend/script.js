...
document.getElementById('reveal-question-btn').addEventListener('click', () => {
  // Now redundant but kept for safety
  modalInitialView.classList.add('hidden');
  modalQuestionView.classList.remove('hidden');
});
...
function checkPath(teamId) {
  ...
  while (queue.length > 0) {
    ...
    // Check win condition
    if (teamId === 1 && parseInt(curr.c) === 6) return true; // Red: Left to Right (0 to 6)
    if (teamId === 2 && parseInt(curr.r) === 3) return true; // Blue: Top to Bottom (0 to 3)
